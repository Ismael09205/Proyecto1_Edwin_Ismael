...
public class PruebaPsicometricaDAO implements Persistible<PruebaPsicometrica> {
    ...
    private Long insertar(PruebaPsicometrica prueba) throws BaseDatosException {
        ...
        try {
            ...
            // fecha_realizacion (CORREGIDO: índice 8)
            if (prueba.getFechaRealizacion() != null) {
                LocalDateTime fecha = prueba.getFechaRealizacion();
                stmt.setDate(8, java.sql.Date.valueOf(fecha.toLocalDate()));
            } else {
                stmt.setDate(8, new java.sql.Date(System.currentTimeMillis()));
            }

            // aprobado (CORREGIDO: parámetro 9)
            stmt.setBoolean(9, prueba.isAprobado() != null ? prueba.isAprobado() : false);
    ...
    public PruebaPsicometrica obtenerUltimaPruebaAprobada(Long conductorId) throws BaseDatosException {
        ...
        try {
            ...
            while (rs.next()) {
                PruebaPsicometrica prueba = mapearResultSet(rs);
                try {
                    if ("APROBADO".equalsIgnoreCase(prueba.obtenerEstado())) {
                        return prueba;
                    }
                } catch (Exception ignore) {
                    // si obtenerEstado falla, seguimos con la siguiente fila
                }
            }
    ...
    private PruebaPsicometrica mapearResultSet(ResultSet rs) throws SQLException {
        ...
        // Mapear según columnas de la tabla
// puntaje_visual  -> notaPercepcion
